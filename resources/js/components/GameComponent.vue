<template>
	<div>
		<draggable class="itemList" :options="options" :group="{ name: 'items', pull: 'clone', put: false }" 
      >
        <div
            class="item"
            ...
            :key="index"
        >
            <p><img :src="'/img/item/' + item.path"/></p>
          <p>{{ item.name }}</p>
        </div>
      </draggable>
      <draggable class="itemList itemList2" :options="options" group="items" @add="onAdd"
      >
      </draggable>
	</div>
</template>
